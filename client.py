...
class fileMgmt():
    ...
    def SockInit(self):
        ...
        for key,value in self.cDtls.ServerIPs.items():
            try:
                ...
                print("Problem Connecting Server {}. Trying to connect other servers".format(key))
            ...
            for key,value in sockList.items():
                ...
                try:
                    if self.ServerStat[key] == 'OFF':
                        print("Unable to send to Server {}. Server might be down".format(key))
                        continue
                    ...
                    data2send = self.prepareHeader(int(OffsetLen[(parts2send[0]-1)]),parts2send[0],
                                                   filename,fsize,fileOffset[(parts2send[0]-1)])
                    #encrypt_dat1 = self.encryptPart(data2send.encode())
                    value.send((data2send+'-'*(100-len(data2send))).encode())#encrypt_dat1 MACHI HERE YOU ARE SENDING LEN+8 AS LENGTH CORRECT AGI NODU
                    ...
                    while BytesSent>0:
                        ...
                        while not Recv_ACK:
                            try:
                                while status =='':
                                    status=value.recv(1)
                                    print("Sending file part "+str(parts2send[0])+" : "+\
                                              str(100-int((BytesSent/int(OffsetLen[(parts2send[0]-1)])*100)))+"%\r",end='')
                                Recv_ACK=True
                            except timeout:
                                value.send(encrypt_dat11)
                                time.sleep(0.01)
                    ...
                    data2send2 = self.prepareHeader(int(OffsetLen[(parts2send[1]-1)]),parts2send[1],
                                                    filename,fsize,fileOffset[(parts2send[1]-1)])
                    ...
                    while BytesSent>0:
                        ...
                        while not Recv_ACK:
                            try:
                                ...
                                value.send(encrypt_dat22)
                                time.sleep(0.01)
                        print("Sending file part "+str(parts2send[1])+" : "+\
                                          str(100-int((BytesSent/int(OffsetLen[(parts2send[1]-1)])*100)))+"%\r",end='')
                        BytesSent = BytesSent -len(data)
                    print("Sending file part "+str(parts2send[1])+" : 100%") 
                    #value.settimeout(None)
                except timeout:
                    print("Server is not responding")
                    print("Problem while transferring file to Server {}".format(key+1))
                value.settimeout(None)
                    
    def prepareHeader(self,data,parts2send,filename,size,offset):
        data2send2 = self.cDtls.username+"\n"+self.cDtls.passwd +"\n"+filename+"\n"+ \
                    str(parts2send)+"\n"+str(data)+"\n"+str(size)+"\n"+str(offset)+"\n"
        return data2send2
            ...
            while i<2:
                ...
                with open(subf+"Rx_"+file1+part,'wb') as fh:
                    while bytesrecv < int(len1):
                        ...
                        sock.send("s".encode())
                        fh.write(ucsm)
                        print("Recieving part : {} => {}%\r".format(part,
                                                                    (int((bytesrecv/int(len1))*100))),end='')
    ...
    def decryptPart(self,data):
        nonc = data[:8]
        encrypted = data[8:]
        decryption_suite = Salsa20.new(PASSWD.encode(),nonce=nonc)#b'0123456789012345',nonce=nonc)
    ...
    def Clear_the_Mess(self,file):
        ...
        if len(sub)>1:
            ...
            subf = '' 
            file1=file
        if os.path.isfile(subf+"Rx_"+file1):
            os.remove(subf+"Rx_"+file1)
        print("Writing to => "+subf+"Rx_"+file1)
        with open(subf+"Rx_"+file1,'ab') as fh:
            for each in sorted(recvList.keys()):
                with open(recvList[each],'rb') as tmp:
                    lines=tmp.readlines()
                    for every in lines:
                        fh.write(every)
                os.remove(subf+"Rx_"+file1+each)
...
class configMgmt():
    ...
    def getServerDetails(self):
        try:
            ...
            print("There was problemm accessing the file. \
...
if __name__ == '__main__':
    ...
    argparser.add_argument('configFile',action='store',type=str,help=" Enter the configuration file for client")
    if (len(sys.argv)!=2):
        print("Invalid arguements, you need to enter config file name. program now exits")
        sys.exit()
